...
export default function GameSessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  useEffect(() => {
    if (!sessionData || sessionData.mode !== "blitz" || result || timedOut) return
    const interval = setInterval(() => {
  ...
  const handleTimeout = useCallback(async () => {
    ...
    await fetch(`/api/session/${sessionId}/submit`, {
      ...
      body: JSON.stringify({ caseId: c.id, answer: -1, responseTimeMs: 30_000 }),
    })
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch(`/api/session/${sessionId}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ caseId: c.id, answer: selected, responseTimeMs }),
      })
      if (!res.ok) return
  ...
  async function finishSession() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        sessionStorage.setItem(`rxarena_result_${sessionId}`, JSON.stringify(data))
          ...
          <div className="flex items-center gap-3">
            ...
            {isBlitz && (
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${
                blitzTimeMs <= 10_000 ? "bg-destructive/10 text-destructive" : "bg-muted"
              }`}>
                <Clock className="w-4 h-4" />
                <span className="text-sm font-mono font-medium">{formatTime(blitzTimeMs)}</span>
              </div>
            )}

            {isCompetitive && timeRemainingMs !== null && (
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${
                timeRemainingMs <= 60_000 ? "bg-destructive/10 text-destructive" : "bg-muted"
              }`}>
                <Clock className="w-4 h-4" />
        ...
        <div className="max-w-4xl mx-auto mt-3">
          <div className="h-1 bg-muted rounded-full overflow-hidden">
            <div
              ...
              style={{ width: `${((currentIndex + (result ? 1 : 0)) / sessionData.cases.length) * 100}%` }}
            />
          </div>
        </div>
      </header>
      ...
      <div className="p-4 md:p-8 max-w-4xl mx-auto">
        ...
        {timedOut && (
          <div className="mb-4 p-3 rounded-xl bg-destructive/10 border border-destructive/20">
            <p className="text-sm font-medium text-destructive">Time&apos;s up! Moving to next question…</p>
          </div>
        )}
        ...
        <div className="space-y-3 mb-6">
          {currentCase.options.map((option, index) => {
            ...
            if (result || timedOut) {
              if (isCorrectAnswer && isPractice) cardClass += " bg-emerald-500/10 border-2 border-emerald-500"
              else if (isWrongSelected) cardClass += " bg-destructive/10 border-2 border-destructive"
            } else if (isSelected) {
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-foreground mb-1">
                  {result.isCorrect ? `Correct! +${result.scoreAwarded} pts` : "Incorrect"}
                </p>
        ...
        {result && !isPractice && (
          <div className={`mb-6 p-3 rounded-xl text-center ${result.isCorrect ? "bg-emerald-500/10" : "bg-muted"}`}>
            <p className={`text-sm font-medium ${result.isCorrect ? "text-emerald-600" : "text-muted-foreground"}`}>
              {result.isCorrect ? `+${result.scoreAwarded} pts` : "Incorrect — 0 pts"}
            </p>
          </div>
        )}
        ...
        {result && isPractice && (
          <Button onClick={handleNext} className="w-full h-12" size="lg">
            {currentIndex + 1 >= sessionData.cases.length ? "See Results" : "Next Question"}
